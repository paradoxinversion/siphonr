...
class UserSearchBar extends React.Component {
    ...
    return (
      <form className="user-search-form" onSubmit={this.handleSubmit}>
        ...
        <select
          ...
          name="tweetsAmt"
          value={this.state.tweetsAmt}
          onChange={this.handleUserQueryUpdate}>
          <option className="dropdown-item" value={25}>
            25
          </option>
          ...
          <option className="dropdown-item" value={200}>
            200
          </option>
  ...
  render() {
    if (this.props.isAuthenticated) {
      return <div className="form-container">{this.renderSearchForm()}</div>;
    } else {
      return null;
    }
  }
